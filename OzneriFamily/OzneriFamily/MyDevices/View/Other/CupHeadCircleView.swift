...
class CupHeadCircleView: UIView {

    
    // Only override draw() if you perform custom drawing.
    // An empty implementation adversely affects performance during animation.
    
    private var lastAngle:CGFloat=0//lastAngle=0需要动画，否则不需要动画
    ...
    override func draw(_ rect: CGRect) {
        ...
        shapeLayer.path=UIBezierPath(arcCenter: CGPoint(x: rect.size.width/2, y: rect.size.width/2), radius: rect.size.width/2-5, startAngle: -CGFloat(M_PI), endAngle: -CGFloat(M_PI)+CGFloat(M_PI)*currentAngle, clockwise: true).cgPath
    ...
    func updateCircleView(angle:CGFloat){
        currentAngle=angle
