...
class DeviceViewContainer: UIView {
    ...
    private func SelectWitchView(device:OznerBaseDevice?)  {
        ...
        if device==nil {
            ...
            case .TwoCup:
                delegate.WhitchCenterViewIsHiden!(SettingIsHiden: false, BateryIsHiden: true, FilterIsHiden: true,BottomValue:237*k_height)
            case .Tap:
    ...
    func OznerDeviceSensorUpdate(identifier: String) {
        DispatchQueue.main.async {
            ...
            case .WaterPurifier_Wifi:
                ...
                if currentDevice?.deviceInfo.wifiVersion == 3 {
                    ...
                    self.LvXinValue =  lroundf(Float(min(filterA, filterB, filterC)))
                }
