...
- (void)sendTextMessageWithContent:(NSString *)messageStr
{
    ...
    [User  sendMessageToKeFuWithMessage:messageStr stateBlock:^(BOOL isBool) {
...
#pragma mark - 消息发送状态修改

- (void)messageSendFaild:(ICMessageFrame *)messageF
{
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        messageF.model.message.deliveryState = ICMessageDeliveryState_Failure;
        
        ICMessageFrame *msgF = [ICMessageHelper createMessageFrame:TypeSystem content:@"发送失败,请确保网络通畅并已关注浩泽微信公众号!" path:nil from:@"gxz" to:self.group.gId fileKey:nil isSender:YES receivedSenderByYourself:NO];
        [self.dataSource addObject:msgF];
        [self.tableView reloadData];
    });
}
