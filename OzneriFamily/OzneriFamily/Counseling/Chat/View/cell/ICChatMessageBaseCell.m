...
- (void)setModelFrame:(ICMessageFrame *)modelFrame
{
    ...
    if (messageModel.isSender) {    // 发送者
        ...
        [self.headImageView.imageView sd_setImageWithURL:[NSURL URLWithString:User.currentUser.headimage]];
    } else {    // 接收者
        self.retryButton.hidden  = YES;
        self.bubbleView.image    = [UIImage imageNamed:@"liaotianbeijing1"];
        [self.headImageView.imageView setImage:[UIImage imageNamed:@"loginLogo"]];
    }
}
